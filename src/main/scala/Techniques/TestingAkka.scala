...
class TestingAkka extends TestKit(ActorSystem("TestingAkkaStream")) with WordSpecLike with BeforeAndAfterAll {
  ...
  "A simple stream" should {
    ...
    "integrate woth streams testkit Sink" in {
    ...
    "test flows with a test source and a test sink" in {
      ...
      val (pub, sub) = materialized

      pub.sendNext(1).sendNext(5).sendNext(42).sendNext(99).sendComplete()
      sub.request(4).expectNext(2,10,84,198).expectComplete()
    }
  }
}
